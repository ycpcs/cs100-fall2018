...
# TODO 2: implement the code for the following comment
    ...
    if (x >= x1 and x <= x2) and (y <= y1) and (y >= y2):
        print("collision with rectangle: ", "x1:", x1, "y1:", y1, "x2:", x2, "y2:", y2)
        return True
    else:
        return False
...
# TODO 3: implement the code for the following comment
    ...
    if sqrt((x - x1)**2 + (y - y1)**2) < radius:
        print("collision with circle: ", "x1:", x1, "y1:", y1, "radius:", radius)
        return True
    else:
        return False
...
# TODO 5: surround the prompt below with a loop that validates the user input
    # get maxMove from user
    # validate maximum move value and keep prompting until a valid value is entered
    ...
    while (maxMove < 20) or (maxMove > 200):
        ...
        # if not valid, issue prompt
        if (maxMove < 20) or (maxMove > 200):
    ...
    # while user has not collided with too many obstacles
...
# TODO 6: add collision detection for circle3 and circle4
            # if no collision, check for collision with circle3
            if not collision:
                collision = detectCollisionWithCircle(bob, circle3)

            # if no collision, check for collision with circle4
            if not collision:
...
# TODO 7: add collision detection for rectangle2
            # if no collision, check for collision with rectangle2
            if not collision:
...
# TODO 9: resets the distance for the next move
        ...
        if distance >= maxMove:
            # prompt user for new heading
            print("Time for a new move - enter new heading: ")
